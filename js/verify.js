...
(function () {
  ...
  function wireDownloadButtons() {
    ...
    document.getElementById('copy-link-btn').addEventListener('click', async () => {
      if (!currentRow) return;
      try {
        await CertDownload.copyVerifyLink(currentRow);
        flashButton('copy-link-btn', 'Copied!');
      } catch (err) {
  ...
  // ---- Populate the verify URL box below the download button ----
  function populateVerifyUrlBox(row) {
    const box   = document.getElementById('verify-url-box');
    const input = document.getElementById('verify-url-input');
    const link  = document.getElementById('open-verify-link');
    if (!box || !input || !link) return;
